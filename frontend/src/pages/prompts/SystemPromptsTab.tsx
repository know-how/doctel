/**
 * SystemPromptsTab.tsx - System Prompts by Task Type
 *
 * Extracted from AdminPromptsPage for use in the merged PromptManagementPage.
 * Manages system prompt templates grouped by type (chat/summary/extraction/...).
 * Supports inline editing, version history, and test simulation.
 */
...
export const SystemPromptsTab: React.FC = () => {
  ...
  const handleTest = async (_promptType: string, _content: string) => {
    try {
      setTesting(true)
      setError(null)
      setTestOutput(`[Test output for "${_promptType}" prompt with input: "${testInput || '(no input provided)'}"]
      ...
      {showNew && (
        ...
        >
          <div style={{ marginBottom: 8 }}>
            ...
            >
              {PROMPT_TYPES.map((pt) => (
                <option key={pt} value={pt} style={{ backgroundColor: c.bgSecondary, color: c.text }}>
                  {pt}
                </option>
              ))}
            </select>
        ...
        <div style={{ display: "grid", gap: t.spacing.md }}>
          {filtered.map((p) => (
            ...
            >
                ...
                <>
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button
                        onClick={() =>
                          setShowVersions(showVersions === p.id ? null : p.id)
                        }
                        style={{
                          padding: "4px 10px",
                          borderRadius: t.radii.sm,
              ...
              {/* Test prompt section (only when editing) */}
              {editingId === p.id && (
                <div
                  style={{
                    marginTop: 12,
